#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# DDWS1 = DDWS1_phylomorpho_of_all_data+depthDataSubsets+convexHullGraphs
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
depth.201TO300$Species <- trimws(depth.201TO300$Species)
...
depth.601TO700$Species <- trimws(depth.601TO700$Species)
#depth.701TO800 <- subset(depth.data,depth.data$Depth>701 & depth.data$Depth<=800)
depth.800plus <- subset(depth.data,depth.data$Depth>801)
depth.800plus$Species <- trimws(depth.800plus$Species)
...
depth.201TO400$Species <- trimws(depth.201TO400$Species)
...
depth.601TO800$Species <- trimws(depth.601TO800$Species)
depth.800plus <- subset(depth.data,depth.data$Depth>801)
depth.800plus$Species <- trimws(depth.800plus$Species)

# create new df based on the species from phylomorphospace
depth.data.filtered <- rbind(depth.201TO300,depth.601TO700,depth.800plus)
list.of.depth.data.100s <- list(depth.201TO300,depth.601TO700,depth.800plus)
list.of.depth.data.200s <- list(depth.201TO400,depth.601TO800,depth.800plus)
...
a3 <- calcConvex.phylomorpho.chull(depth.201TO300,"Depth: 201-300",new.tree,"Species")
...
a7 <- calcConvex.phylomorpho.chull(depth.601TO700,"Depth: 601-700",new.tree,"Species")
#a8 <- calcConvex.phylomorpho.chull(depth.701TO800,"Depth: 701-800",new.tree)
a9 <- calcConvex.phylomorpho.chull(depth.800plus,"Depth: 800+",new.tree,"Species")

# calculate and plot the areas for the 100s
area.val1 <- c(a3[[2]],a7[[2]],a9[[2]])
dev.new()
...
b2 <- calcConvex.phylomorpho.chull(depth.201TO400,"Depth: 201-400",new.tree,"Species")
#b3 <- calcConvex.phylomorpho.chull(depth.401TO600,"Depth: 401-600",new.tree)
b4 <- calcConvex.phylomorpho.chull(depth.601TO800,"Depth: 601-800",new.tree,"Species")
b5 <- calcConvex.phylomorpho.chull(depth.800plus,"Depth: 800+",new.tree,"Species")

# calculate and plot the areas for the 200s
area.val2 <- c(b2[[2]],b4[[2]],b5[[2]])
dev.new()
...
write.csv(depth.data.filtered, "DDWS.depth.data.filtered_OUTPUTS.csv")

### RData
saveRDS(object = list.of.depth.data.100s, file = "DDWS.listOfDepthData100s_OUTPUTS.RData")
saveRDS(object = list.of.depth.data.200s, file = "DDWS.listOfDepthData200s_OUTPUTS.RData")

#saveRDS(object = depth.0TO100, file = "DDWS.depth.0TO100_OUTPUTS.RData")
#saveRDS(object = depth.101TO200, file = "DDWS.depth.101TO200_OUTPUTS.RData")
saveRDS(object = depth.201TO300, file = "DDWS.depth.201TO300_OUTPUTS.RData")
#saveRDS(object = depth.301TO400, file = "DDWS.depth.301TO400_OUTPUTS.RData")
#saveRDS(object = depth.401TO500, file = "DDWS.depth.401TO500_OUTPUTS.RData")
#saveRDS(object = depth.501TO600, file = "DDWS.depth.501TO600_OUTPUTS.RData")
saveRDS(object = depth.601TO700, file = "DDWS.depth.601TO700_OUTPUTS.RData")
#saveRDS(object = depth.701TO800, file = "DDWS.depth.701TO800_OUTPUTS.RData")
saveRDS(object = depth.800plus, file = "DDWS.depth.800plus_OUTPUTS.RData")

#saveRDS(object = depth.0TO200, file = "DDWS.depth.0TO200_OUTPUTS.RData")
saveRDS(object = depth.201TO400, file = "DDWS.depth.201TO400_OUTPUTS.RData")
#saveRDS(object = depth.401TO600, file = "DDWS.depth.401TO600_OUTPUTS.RData")
saveRDS(object = depth.601TO800, file = "DDWS.depth.601TO800_OUTPUTS.RData")

### PDF
pdf(file= "DDWS1.cHullAreas100s+cHullAreas100s_OUTPUTS.pdf")
plot(area.val1,main="Depth Range vs CHull Area by the 100s",xlab="Depth Range (ft)",ylab="CHull Area", type = "l")
plot(area.val2,main="Depth Range vs CHull Area by the 200s",xlab="Depth Range (ft)",ylab="CHull Area", type = "l")
dev.off() 
