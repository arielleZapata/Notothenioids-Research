#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# DDWS4 = DDWS4_iqrLengthVsDepthRange
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
colnames(iqr.100s) <- list("201to300","601to700","800+")
iqr.100s <- t(iqr.100s)
iqr.100s <- data.frame(iqr.100s)
...
colnames(iqr.200s) <- list("201to400","601to800","800+")
iqr.200s <- t(iqr.200s)
iqr.200s <- data.frame(iqr.200s)
...
## Outputs
### PDF
pdf(file= "DDWS4.plot100sIQR+plot200sIQR_OUTPUTS.pdf")
ggplot(iqr.100s,aes(x = x , y = y))+ 
  geom_segment( aes(x=x, xend=x, y=0, yend=y))+
  geom_point( size=5, color="blue", fill=alpha("green", 0.3), alpha=0.7, shape=21, stroke=2) 
