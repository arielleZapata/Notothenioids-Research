...
list.of.depth.data.100s <- readRDS("~/Notothenioids_research/repository/all_analysis_data_n_code/p2_pholomorpho+depthPlots/listOfDepthData100s_OUTPUTS.RData")
list.of.depth.data.200s <- readRDS("~/Notothenioids_research/repository/all_analysis_data_n_code/p2_pholomorpho+depthPlots/listOfDepthData200s_OUTPUTS.RData")
...
colnames(iqr.100s) <- list("0to100","101to200","201to300","301to400","401to500","501to600","601to700","701to800","800+")
iqr.100s <- t(iqr.100s)
iqr.100s <- data.frame(iqr.100s)
...
colnames(iqr.200s) <- list("0to200","201to400","401to600","601to800","800+")
iqr.200s <- t(iqr.200s)
iqr.200s <- data.frame(iqr.200s)
...
## Outputs
### PDF
pdf(file= "plot100sIQR+plot200sIQR_OUTPUTS.pdf")
ggplot(iqr.100s,aes(x = x , y = y))+ 
  geom_segment( aes(x=x, xend=x, y=0, yend=y))+
  geom_point( size=5, color="blue", fill=alpha("green", 0.3), alpha=0.7, shape=21, stroke=2) 
