...
replace_and_filter_data <- function(data, mapping) {
  ...
  for (i in 1:length(data$Species)) {
    for (j in 1:length(mapping$Polarstern)) {
      if (data$Species[i] == mapping$Polarstern[j]) {
        data$Species[i] <- mapping$New[j]
      }
    }
  }
  
  # Filter data to keep only rows with species in the new list
  data <- data[data$Species %in% mapping$New, ]
  
  return(data)
}

# Replace and filter species names in more.depth.data
more.depth.data <- replace_and_filter_data(more.depth.data, depthNameChange)
more.depth.data$Depth <- more.depth.data$Depth..m.
more.depth.data$Length <- more.depth.data$TL..cm.
depth.data$Species <- depth.data$Taxon

depth.data <- merge(depth.data, more.depth.data, by.x = c('Species', 'Depth','Length'), by.y = c('Species', 'Depth','Length'), all = TRUE)
...
depth.greaterThan801 <- subset(depth.data,depth.data$Depth>=801)
...
depth.data.filtered <- rbind(depth.0TO100,depth.101TO200,depth.201TO300,depth.301TO400,depth.401TO500,depth.501TO600,depth.601TO700,depth.701TO800,depth.greaterThan801)
list.of.depth.data.100s <- list(depth.0TO100,depth.101TO200,depth.201TO300,depth.301TO400,depth.401TO500,depth.501TO600,depth.601TO700,depth.701TO800,depth.greaterThan801)
list.of.depth.data.200s <- list(depth.0TO200,depth.201TO400,depth.401TO600,depth.601TO800,depth.greaterThan801)
...
a9 <- calcConvex.phylomorpho.chull(depth.greaterThan801,"Depth: 801-MAX",new.tree,dfName="Species")

# calculate and plot the areas for the 100s
area.val1 <- c(a1[[2]],a2[[2]],a3[[2]],a4[[2]],a5[[2]],a6[[2]],a7[[2]],a8[[2]],a9[[2]])
dev.new()
...
b5 <- calcConvex.phylomorpho.chull(depth.greaterThan801,"Depth: 801-MAX",new.tree,dfName="Species")

# calculate and plot the areas for the 200s
area.val2 <- c(b1[[2]],b2[[2]],b3[[2]],b4[[2]],b5[[2]])
dev.new()
